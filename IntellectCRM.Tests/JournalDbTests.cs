...
public class JournalDbTests
{
    ...
    /// <summary>Guruhga biriktirilgan yozma topshiriq (muddat va kechikish siyosati bilan).</summary>
    private static Assignment AddAssignment(TestDb db, Group g, string? dueDate = null,
        bool lateAccept = false, int latePenaltyPct = 0)
    {
        var a = new Assignment
        {
            ...
            Title = "Uy vazifasi 1",
            Format = "written",
            ClassIds = new List<string> { g.Id },
            DueDate = dueDate,
            LateAccept = lateAccept,
            LatePenaltyPct = latePenaltyPct,
            MaxScore = 100,
        };
        db.Context.Assignments.Add(a);
        db.Context.SaveChanges();
        return a;
    }

    [Fact]
    public async Task Assignment_NATIJALARClassNameMATNIboyicha_M2MazoKORINMAYDI_HOZIRGIXULQ()
    {
        // HOZIRGI xulqni muzlatadi — pastdagi Skip'li test to'g'ri xulqni tasvirlaydi.
        using var db = TestDb.Sqlite();
        var g = AddGroup(db);
        AddStudent(db, g, "M2M a'zo");   // Student.ClassName bo'sh, faqat StudentGroup a'zoligi bor
        var a = AddAssignment(db, g);

        var res = await AssignmentService.GetResultsAsync(db.Context, a.Id);

        Assert.Empty(res!.Rows);
    }

    [Fact(Skip = "XATO (AssignmentService.cs:115-118): natijalar ClassName MATNI bo'yicha — M2M a'zo ko'rinmaydi")]
    public async Task Assignment_NATIJALAR_M2Mazolikboyicha_OLINISHIKERAK()
    {
        // XATO (AssignmentService.cs:115-118): o'quvchilar `Students.ClassName` MATNI guruh nomiga
        // teng bo'lgani bo'yicha tanlanadi. Markazda a'zolik esa M2M jadval (`StudentGroup`) orqali
        // yuritiladi va ko'p o'quvchida `ClassName` umuman bo'sh (yoki boshqa guruh nomi) —
        // natijada topshiriq natijalari jadvali BO'SH ko'rinadi yoki noto'g'ri o'quvchilarni
        // ko'rsatadi. Guruh nomi o'zgartirilsa ham bog'lanish uziladi.
        // To'g'risi: `StudentGroups` (IsActive, Status != "frozen") bo'yicha olinishi kerak —
        // TestResultService.DetailAsync bilan bir xil qoida.
        using var db = TestDb.Sqlite();
        var g = AddGroup(db);
        AddStudent(db, g, "M2M a'zo");
        var a = AddAssignment(db, g);
    ...
    [Fact]
    public async Task Assignment_MUDDATOTGACHHAMTOLIQBALL_HOZIRGIXULQ()
    {
        // HOZIRGI xulqni muzlatadi — pastdagi Skip'li test to'g'ri xulqni tasvirlaydi.
        using var db = TestDb.Sqlite();
        var g = AddGroup(db);
        var s = AddStudent(db, g, "Ali");
        // Muddat 5 kun oldin tugagan, kechikkanni qabul qilish O'CHIQ.
        var a = AddAssignment(db, g, dueDate: Day(-5), lateAccept: false);

        var r1 = await AssignmentService.SubmitAsync(db.Context, a.Id, g.Id, s.Id,
            new SubmitAssignmentRequest(null, "Kechikkan javob", null));
        var r2 = await AssignmentService.SubmitAsync(db.Context, a.Id, g.Id, s.Id,
            new SubmitAssignmentRequest(null, "Yana bir marta", null));

        Assert.True(r1!.Completed);
        Assert.True(r2!.Completed);   // urinishlar soni ham cheklanmagan
    }

    [Fact(Skip = "XATO (AssignmentService.cs:267-300): DueDate/LateAccept/LatePenaltyPct QO'LLANMAYDI")]
    public async Task Assignment_MUDDATOTGACH_LateAccessOCHIQBOLSA_RADETILISHIKERAK()
    {
        // XATO (AssignmentService.cs:267-300): SubmitAsync faqat topshiriq mavjudligini va
        // guruhga tegishliligini tekshiradi. `StartDate`, `DueDate`, `LateAccept` va
        // `LatePenaltyPct` maydonlari saqlanadi, formada sozlanadi va o'quvchi ilovasida
        // ko'rsatiladi — lekin BAHOLASHDA umuman ishlatilmaydi: muddat o'tgach ham to'liq ball
        // beriladi, kechikkanlik jarimasi qo'llanmaydi va qayta-qayta topshirish ochiq
        // (test formatida — kalitni bilib olib 100% olish mumkin).
        // To'g'risi: LateAccept=false bo'lsa muddatdan keyingi topshirish RAD etilishi,
        // LateAccept=true bo'lsa ball LatePenaltyPct ga kamaytirilishi kerak.
        using var db = TestDb.Sqlite();
        var g = AddGroup(db);
        var s = AddStudent(db, g, "Ali");
        var a = AddAssignment(db, g, dueDate: Day(-5), lateAccept: false);

        var r = await AssignmentService.SubmitAsync(db.Context, a.Id, g.Id, s.Id,
            new SubmitAssignmentRequest(null, "Kechikkan javob", null));
